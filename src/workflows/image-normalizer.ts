...
const RotationInstructionsSchema = z.object({
	currentAssumedClockwiseRotation: z.enum(["0", "90", "180", "270"], {description: "If the photo is not in it's intended position, determine what clockwise rotation was made previously to get it into it's current state. Use the text in the image to help determine the rotation. Take your time to think carefully."}),
	degreesToRotate: z.enum(["0", "90", "180", "270"], {
		description: "The amount of degrees of clockwise rotation needed to make the photo upright. Ensure the image is not upside down, the text should be right side up. Take your time in ensuring the poster is not upside down."})
});
...
export class ImageNormalizer extends WorkflowEntrypoint<Env, NormalizerParams> {
	async run(event: Readonly<WorkflowEvent<NormalizerParams>>, step: WorkflowStep): Promise<string> {
		...
		const rotationInstructions = await step.do(`Determine image rotation needs for ${uploadedImageUrl}`, async() => {
			...
			const completion = await oai.beta.chat.completions.parse({
			  ...
			  response_format: zodResponseFormat(RotationInstructionsSchema, "rotationInstructions"),

			});
			const rotationInstructions = completion.choices[0].message.parsed as RotationInstructions;
			console.log("rotationInstructions", JSON.stringify(rotationInstructions));
			return rotationInstructions;
		});
		const transformedImageUrl = await step.do("Rotate and store image", async() => {
			...
			const result = await this.env.IMAGES.input(stream)
				.transform({rotate: parseInt(rotationInstructions?.degreesToRotate as string)})
				.transform({fit: "scale-down"})
				.transform({width: 640})
				.output({format: "image/png"});
			const transformedFileName = await safeFileNameForPrefix(this.env, slug as string)
